...
2. **Check for Existing .autoo Files:**
    - Look for existing `.autoo/feature_list.json`, `.autoo/progress.md`, or other .autoo files
...
3. **Handle Missing spec.txt:**
    - If `.autoo/spec.txt` doesn't exist (legacy codebase), create one based on your analysis of the existing codebase
...
### STEP 3: Analyze Existing Codebase and Create .autoo/feature_list.json
...
2. **Populate .autoo/feature_list.json:**
   Based on `.autoo/spec.txt` AND your analysis of the existing codebase, update `.autoo/feature_list.json` with 20 detailed end-to-end test cases.
...
- Update `.autoo/feature_list.json` to include any uncovered issues, technical debt, or improvements discovered during analysis
...
2. Update `.autoo/progress.md` with a summary of what you accomplished and the current state of the codebase
3. Ensure .autoo/feature_list.json accurately reflects the existing codebase including discovered issues and improvement opportunities
4. Ensure .autoo/project_structure.md exists and documents the current architecture
