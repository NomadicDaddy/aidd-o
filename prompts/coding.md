...
- **Spec (source of truth):** `/.autoo/spec.txt`
- **Architecture map:** `/.autoo/project_structure.md`
- **Feature tests checklist:** `/.autoo/feature_list.json`
- **Todo list:** `/.autoo/todo.md`
- **Progress log:** `/.autoo/progress.md`
- **Project overrides (highest priority):** `/.autoo/project.txt`
- **Tools (canonical):** `/.autoo/tools.md`

### HARD CONSTRAINTS

1. **Do not run** `scripts/setup.ts`. Setup was performed by the initializer session.
2. If there is a **blocking ambiguity** or missing requirements, **stop** and record the question in `/.autoo/progress.md`.
...
1. **Core Models Verification:**
    - Read `/.autoo/spec.txt` to identify required data models (e.g., Todo, User, Tag)
...
3. **Feature List Alignment:**
    - Cross-reference `/.autoo/feature_list.json` with the spec
...
ls -la
cat .autoo/spec.txt
head -50 .autoo/feature_list.json
# Create progress.md if missing - initialize with session info
if [ ! -f .autoo/progress.md ]; then
  echo "PROGRESS TRACKING INITIALIZED: $(date)" > .autoo/progress.md
  echo "Session start: New context window" >> .autoo/progress.md
fi
cat .autoo/progress.md
git log --oneline -20
grep '"passes": false' .autoo/feature_list.json | wc -l
```
...
Get-Location
Get-ChildItem -Force
Get-Content .autoo/spec.txt
Get-Content .autoo/feature_list.json -TotalCount 50
# Create progress.md if missing - initialize with session info
if (-not (Test-Path .autoo/progress.md)) {
  "PROGRESS TRACKING INITIALIZED: $(Get-Date)" | Out-File .autoo/progress.md
  "Session start: New context window" | Add-Content .autoo/progress.md
}
Get-Content .autoo/progress.md
...
(Select-String -Path .autoo/feature_list.json -Pattern '"passes"\s*:\s*false').Count
```

Understanding the `/.autoo/spec.txt` is critical - it contains the full requirements for the application you're building.
...
- **Always create `/.autoo/progress.md` if missing** - initialize with current session timestamp.
...
Always follow `/.autoo/project.txt` overrides if present.
...
2. **Feature Integrity Audit:**
    - Review `/.autoo/feature_list.json` for accuracy
...
Check for the existence of a todo list for priority work - `/.autoo/todo.md` and intelligently ingest each entry into `/.autoo/feature_list.json` (THIS IS THE ONLY TIME YOU MAY ADD TO THIS FILE) and then remove each item from the todo list. It should be empty or deleted when complete.

Look at `/.autoo/feature_list.json` and find the highest-priority feature with "passes": false.
...
### STEP 9: UPDATE /.autoo/feature_list.json (CAREFULLY!)
...
git commit -m "Implement [feature name] - verified end-to-end" \
  ...
  -m "- Updated /.autoo/feature_list.json: marked test #X as passing" \
...
2. Update /.autoo/progress.md
3. Update /.autoo/feature_list.json if tests verified
4. **FINAL FEATURE STATUS VALIDATION:**
    - Perform a final audit of /.autoo/feature_list.json
...
- Zero console errors
- Polished UI matching the design specified in `/.autoo/spec.txt`
...
- **DOCUMENT** any file corruption incidents in `/.autoo/progress.md`
