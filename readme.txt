# autoo - Autonomous Development Orchestrator

A shell script that orchestrates autonomous development sessions using OpenCode AI. autoo supports both bash (autoo.sh) and PowerShell (autoo.ps1) environments.

## Specifications

autoo uses project specifications in Markdown format aligned with Spernakit template standards. All specs are located in the `specs/` directory with `.md` extensions.

## Usage

### Bash/Linux/macOS
```bash
./autoo.sh --project-dir <dir> [--spec <file>] [--max-iterations <num>] [--timeout <seconds>] [--idle-timeout <seconds>] [--model <model>] [--init-model <model>] [--code-model <model>] [--no-clean] [--quit-on-abort <num>]
```

### PowerShell/Windows
```powershell
./autoo.ps1 -ProjectDir <dir> [-Spec <file>] [-MaxIterations <num>] [-Timeout <seconds>] [-IdleTimeout <seconds>] [-Model <model>] [-InitModel <model>] [-CodeModel <model>] [-NoClean] [-QuitOnAbort <num>] [-Help]
```
...
### Optional Arguments
- `--spec` / `-Spec`: Specification file (.md) to copy to project-dir/.autoo/spec.txt
...
3. Copies artifacts into `project-dir/.autoo/`
4. Copies spec file to `.autoo/spec.txt` (only if `--spec` is provided)
...
For directories containing code but no `.autoo/` files:
...
If `.autoo/feature_list.json` exists but still appears to be a template (placeholders like `{yyyy-mm-dd}`), onboarding is treated as incomplete and the onboarding prompt is used again.

### Subsequent Iterations
Once `.autoo/spec.txt` and `.autoo/feature_list.json` exist:
...
2. Checks if both `project-dir/.autoo/spec.txt` AND `project-dir/.autoo/feature_list.json` exist
3. Creates a per-iteration transcript log at `project-dir/.autoo/iterations/001.log`, `002.log`, ...
...
4. If either is missing:
   - Creates `.autoo` directory if needed
...
- `project-dir/.autoo/iterations/001.log`
- `project-dir/.autoo/iterations/002.log`
- ...
...
### New Project Example
```bash
./autoo.sh --project-dir ../new-project --spec ./specs/todolist.md --max-iterations 5
```

### Existing Codebase Example
```bash
./autoo.sh --project-dir ../existing-app --max-iterations 10
```

### PowerShell Examples
```powershell
# New project
./autoo.ps1 -ProjectDir "../new-project" -Spec "./specs/todolist.md" -MaxIterations 5

# Existing codebase
./autoo.ps1 -ProjectDir "../existing-app" -MaxIterations 10
```
...
autoo automatically detects existing codebases by checking for files and directories (excluding common ignored items like `.git`, `.autoo`, `.DS_Store`, `node_modules`, `.vscode`, `.idea`). This enables the appropriate workflow for both new and existing projects.
