...
### Optional Arguments
- `--spec` / `-Spec`: Specification file (.md) to copy to project-dir/.auto/spec.txt
...
3. Copies artifacts into `project-dir/.auto/`
4. Copies spec file to `.auto/spec.txt` (only if `--spec` is provided)
...
For directories containing code but no `.auto/` files:
...
If `.auto/feature_list.json` exists but still appears to be a template (placeholders like `{yyyy-mm-dd}`), onboarding is treated as incomplete and the onboarding prompt is used again.

### Subsequent Iterations
Once `.auto/spec.txt` and `.auto/feature_list.json` exist:
...
2. Checks if both `project-dir/.auto/spec.txt` AND `project-dir/.auto/feature_list.json` exist
3. Creates a per-iteration transcript log at `project-dir/.auto/iterations/001.log`, `002.log`, ...
...
4. If either is missing:
   - Creates `.auto` directory if needed
...
- `project-dir/.auto/iterations/001.log`
- `project-dir/.auto/iterations/002.log`
- ...
...
aidd-o automatically detects existing codebases by checking for files and directories (excluding common ignored items like `.git`, `.auto`, `.DS_Store`, `node_modules`, `.vscode`, `.idea`). This enables the appropriate workflow for both new and existing projects.
